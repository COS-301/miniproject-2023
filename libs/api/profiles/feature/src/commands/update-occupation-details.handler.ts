...
import { IUpdateOccupationDetailsResponse, UpdateOccupationDetailsCommand } from '@mp/api/profiles/util';
import { CommandHandler, EventPublisher, ICommandHandler } from '@nestjs/cqrs';
import { Profile } from '../models';

@CommandHandler(UpdateOccupationDetailsCommand)
export class UpdateOccupationDetailsHandler
  implements ICommandHandler<UpdateOccupationDetailsCommand, IUpdateOccupationDetailsResponse>
{
  ...
  async execute(command: UpdateOccupationDetailsCommand) {
    ...
    if (!request.profile.occupationDetails) throw new Error('Profile occupation details not found');
