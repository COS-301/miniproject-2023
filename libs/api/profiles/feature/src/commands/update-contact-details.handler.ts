...
import { IUpdateContactDetailsResponse, UpdateContactDetailsCommand } from '@mp/api/profiles/util';
import { CommandHandler, EventPublisher, ICommandHandler } from '@nestjs/cqrs';
import { Profile } from '../models';

@CommandHandler(UpdateContactDetailsCommand)
export class UpdateContactDetailsHandler
  implements ICommandHandler<UpdateContactDetailsCommand, IUpdateContactDetailsResponse>
{
  ...
  async execute(command: UpdateContactDetailsCommand) {
    ...
    if (!request.profile.contactDetails) throw new Error('Profile contact details not found');
