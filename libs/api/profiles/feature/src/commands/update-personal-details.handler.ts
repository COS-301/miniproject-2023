...
import { IUpdatePersonalDetailsResponse, UpdatePersonalDetailsCommand } from '@mp/api/profiles/util';
import { CommandHandler, EventPublisher, ICommandHandler } from '@nestjs/cqrs';
import { Profile } from '../models';

@CommandHandler(UpdatePersonalDetailsCommand)
export class UpdatePersonalDetailsHandler
  implements ICommandHandler<UpdatePersonalDetailsCommand, IUpdatePersonalDetailsResponse>
{
  ...
  async execute(command: UpdatePersonalDetailsCommand) {
    ...
    if (!request.profile.personalDetails) throw new Error('Profile personal details not found');
